...
class EvidenceModule extends BaseModule {
    constructor() {
        ...
        // 🔒 Internal Canvas & State Variables
    ...
    setupUIBindings() {
        ...
        getEl('selectVideoBtn')?.addEventListener('click', async () => {
            ...
            // 🛡️ BUG FIX: Replaced @electron/remote with Native IPC
        ...
        getEl('selectFolderBtn')?.addEventListener('click', async (e) => {
            e.preventDefault();
            const currentCase = this.app.getCurrentCase();
            if (!currentCase) return alert("Please open or create an active Case Workspace first.");

            // 🛡️ BUG FIX: Native IPC Folder Selection
            const result = await ipcRenderer.invoke('dialog:showOpenDialog', { 
                properties: ['openDirectory'], 
                title: 'Select Source Evidence Folder' 
            });

            if (!result.canceled && result.filePaths.length > 0) {
                currentCase.sourceFolder = result.filePaths[0];
                if (getEl('folderPathInput')) getEl('folderPathInput').value = result.filePaths[0];
                
                const statusBox = getEl('sortingStatusBox');
                if (statusBox) statusBox.style.display = 'block';
                if (getEl('statusTitle')) { getEl('statusTitle').textContent = "Folder Linked Successfully"; getEl('statusTitle').style.color = "#ffb142"; }
                if (getEl('statusText')) getEl('statusText').textContent = "Ready to parse operations on source directory targets.";
                this.triggerWorkspaceAutoSaveNotification();
            }
        });

        // 🆕 RESTORED: Export Gallery via IPC
        getEl('exportGalleryBtn')?.addEventListener('click', async (e) => {
            ...
            if (!result.canceled && result.filePath) {
                ...
                currentCase.sourceFolder = targetExportPath; 
                if (getEl('folderPathInput')) getEl('folderPathInput').value = targetExportPath;

                if (getEl('statusTitle')) { getEl('statusTitle').textContent = "📦 Export Complete & Linked!"; getEl('statusTitle').style.color = "#2ed573"; }
                if (getEl('statusText')) getEl('statusText').textContent = `Saved to custom location: ${targetExportPath}`;
                if (getEl('sortingStatusBox')) getEl('sortingStatusBox').style.display = 'block';
                
                this.triggerWorkspaceAutoSaveNotification();
            }
        });

        // 🆕 RESTORED: Process Images via IPC
        getEl('processImagesBtn')?.addEventListener('click', async (e) => {
            e.preventDefault();
            const currentCase = this.app.getCurrentCase();
            if (!currentCase || !currentCase.sourceFolder) return alert("Please link a valid source directory.");
            
            const result = await ipcRenderer.invoke('dialog:showSaveDialog', {
                title: 'Choose Location & Name for Sequentially Sorted Folder Output',
                defaultPath: path.join(path.dirname(currentCase.sourceFolder), `${currentCase.ticketId}_Sorted_Output`),
                buttonLabel: 'Save Sorted Output'
            });

            if (!result.canceled && result.filePath) {
                try {
                    if (getEl('statusTitle')) getEl('statusTitle').textContent = "Processing Queue...";
                    const results = currentCase.processEvidence(result.filePath);
                    
                    if (getEl('statusTitle')) { getEl('statusTitle').textContent = "✅ Sorting Optimization Complete!"; getEl('statusTitle').style.color = "#2ed573"; }
                    if (getEl('statusText')) getEl('statusText').textContent = `Successfully processed ${results.count} files into: ${results.destination}`;
                    if (getEl('sortingStatusBox')) getEl('sortingStatusBox').style.display = 'block';
                } catch (err) {
                    if (getEl('statusTitle')) { getEl('statusTitle').textContent = "❌ Process Queue Error Encountered"; getEl('statusTitle').style.color = "#ff4757"; }
                    if (getEl('statusText')) getEl('statusText').textContent = err.message;
                }
            }
        });
    ...
    resetGalleryTimeline() {
        ...
        const sBox = document.getElementById('sortingStatusBox'); if (sBox) sBox.style.display = 'none';
